...
//                       MODE3 MACROS
// ---------------------------------------------------------------------------
#define OFFSET(row, col, width) ((col)+(width)*(row)) // This macro converts 2D screen coordinates into a 1D array index.

#define REG_DISPCNT (*(volatile unsigned short *) 0x4000000)  // This is a pointer to the display control register. You use it to tell the GBA how to draw things (like use Mode 3).
#define MODE3 3
#define BG2_ENABLE (1<<10)  // Set bit 10 of REG_DISPCNT to 1, which enables background layer 2.
...
#define BUTTONS (*(volatile u32 *) 0x4000130) // This memory address stores the current state of all GBA buttons.
#define KEY_DOWN(key, buttons) (~(buttons) & (key)) // Returns true if key is pressed right now
...
#define KEY_JUST_PRESSED(key, buttons, oldbuttons) ((~(oldbuttons) & (buttons)) & (key))  // Returns true if key was just pressed this frame
